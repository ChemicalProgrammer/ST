...
test('the Apps Script browser bundle exposes the tested deterministic engine', () => {
  ...
  assert.equal(typeof browser.SimulatorEngine.calculateConveyorEngineering, 'function');
  assert.equal(browser.SimulatorEngine.runScenarioExperiment, undefined);
  assert.equal(browser.SimulatorEngine.compareScenarioExperiments, undefined);
});
